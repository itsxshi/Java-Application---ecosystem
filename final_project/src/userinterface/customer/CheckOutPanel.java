...
public class CheckOutPanel extends javax.swing.JPanel {
    ...
    public CheckOutPanel(JPanel panel, ShoppingCart cart, CustomerUser user, Network network) {
        initComponents();
        this.panel = panel;
        this.cart = cart;
        this.user = user;
    ...
    private void placeOrderButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_placeOrderButtonActionPerformed
        ...
        String receiverName = nameTextField.getText();
        String receiverAddress = addressTextField.getText();
        String receiverRequirements = commentTextArea.getText();
        String receiverPhone = phoneTextField.getText();

        if (!"".equals(receiverName) && !"".equals(receiverAddress) && !"".equals(receiverPhone)) {
            BuyOrderRequest br = this.network.getWorkqueue().createBuyOrderRequest(user,cart.getFurnitureDir());
            WarehouseRequest wr = this.network.getWorkqueue().createWarehouseRequest(br.getOrderId(),receiverName, receiverAddress, receiverRequirements, receiverPhone, user, cart.getFurnitureDir());
            for (Furniture f : cart.getFurnitureDir().getFurnitureDir()){
                this.system.getNetworkByName(network.getName()).getFurnitureDir().removeFurniture(f,f.getShoppingQuantity());
            }
            DB4OUtil.getInstance().storeSystem(system);

            OrderConfirmationPanel ocPanel = new OrderConfirmationPanel(panel, cart, user, br.getOrderId(), receiverAddress,receiverPhone,receiverName);
